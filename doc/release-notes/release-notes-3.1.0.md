CarbonZero Core version *3.1.0* is now available from:  <https://github.com/carbonzero-project/carbonzero/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/carbonzero-project/carbonzero/issues>


Mandatory Update
==============

CarbonZero Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zcarbon Staking is here! zcarbon staking will be activated on the morning of the 8th of May 2018. With the release of zcarbon staking, there are effectively 2 versions of zcarbon, zcarbon minted on the 3.0.6 CarbonZero wallet or lower, and zcarbon minted on CarbonZero wallet version or higher. New features in this release will require the use of zcarbon v2, zcarbon minted on this wallet release 3.1.0 or later. If you currently hold zcarbon v1 and wish to take advantage of zcarbon staking and deterministic zcarbon, you will need to spend the zcarbon v1 to yourself and remint zcarbon v2.
Note: To find your zcarbon version, click the privacy tab, then the zcarbon Control button then expand the arrows next to the desired denomination.


### Deterministic zcarbon Seed Keys

zcarbon is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zcarbon outside of the wallet that the zcarbon had been minted on. zcarbon can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zcarbon minting

zcarbon minting now only requires 1 further mint (down from 2) to mature. zcarbon mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zcarbon Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zcarbon denomination has been spent or not.



BTZ/zcarbon Staking and Masternode Rewards
--------------

### BTZ, zcarbon and Masternode Payment Schedule
...
- 3 zcarbon (3 x 1 denominations) rewarded to the staker, 2 BTZ rewarded to the masternode owner and 1 BTZ available for the budget. This is a total block reward of 6 BTZ, up from 5.

If a user staking BTZ wins the reward, the following amounts will be rewarded: 
- 2 BTZ to the BTZ staker, 3 BTZ to the Masternode owner and 1 BTZ available for the budget. This is a total block reward of 6 BTZ, up from 5.


### Return change to sender when minting zcarbon

Previously, zcarbon minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most BTZ and return the change from a zcarbon mint to that address.
...
The visual layout of the CarbonZero Qt wallet has undergone a near-complete overhaul.
...
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular BTZ.
...
### Summary
 ...
 The CarbonZero wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the carbonzero.conf file, found in the CarbonZero data directory.
- backuppath = <directory / full path>
- zcarbonbackuppath = <directory / full path>
...
* If zcarbon backup, auto generated name is `wallet-autozcarbonbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
CarbonZero Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The CarbonZero client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running carbonzerod without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the CarbonZero support group, which often times sees users say "I didn't spend that zcarbon". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the CarbonZero masternodes, the monthly budget available to be utilised has been increased to 42,000 BTZ / month. This BTZ only has the opportunity to be raised once per month (paid to winning proposals) with any unused BTZ not created by the blockchain.

### Budget Finalization Fee

The CarbonZero finalization fee for successful proposals has now been reduced, this fee is now 5 BTZ down from 50 BTZ. The total fee outlay for a successful proposal is now a total of 55 BTZ.
...
-	Enforced v1 zcarbon spends to require a security level of 100
-	Updates to zcarbon spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the CarbonZero-Qt masternodes tab
-	Updated CarbonZero-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and CarbonZero's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zcarbon Control dialog in the QT wallet, a user is able to see zcarbon marked as version 1 or 2.

zPoS (zcarbon staking)
--------------

Once a zcarbon has over 200 confirmations it becomes available to stake. Staking zcarbon will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zcarbon. So for example if a 1,000 zcarbon denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zcarbons.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within CarbonZero's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zcarbon, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zcarbon that can be recalculated at any time using the seed. Deterministic zcarbon allows for users to backup all of their future zcarbon by simply recording their seed and keeping it in a safe place (similar to backing up a private key for BTZ). The zcarbon seed needs to remain in the wallet in order to spend the zcarbon after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zcarbon data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

CarbonZero's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which CarbonZero used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to CarbonZero using the bug bounty program. The modulus is now correctly imported and CarbonZero's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zcarbon v2: zPoS dzcarbon ezcarbon external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zcarbon balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zcarbon status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault carbonzero-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zcarbon spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zcarbon. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zcarbon. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/carbonzero-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
