...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZBTZStake)
{
    int64_t ret = 0;
    ...
    if (nHeight <= 43200) {
        ...
        //When zcarbon is staked, masternode only gets 2 BTZ
        ret = 3 * COIN;
        if (isZBTZStake)
            ret = 2 * COIN;
    }
...
bool RecalculateBTZSupply(int nHeightStart)
{
...
bool UpdateZBTZSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateBTZSupply(Params().Zerocoin_StartHeight());
    }

    //Track zcarbon money supply in the block index
    if (!UpdateZBTZSupply(block, pindex))
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BTZ to zcarbon
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZBTZ() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
