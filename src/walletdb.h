...
public:
    ...
    bool WriteZBTZSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBTZSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBTZSeed_deprecated(uint256& seed);
    bool EraseZBTZSeed();
    bool EraseZBTZSeed_deprecated();

    bool WriteZBTZCount(const uint32_t& nCount);
    bool ReadZBTZCount(uint32_t& nCount);
