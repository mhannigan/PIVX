...
CzcarbonWallet::CzcarbonWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcarbon wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzcarbonWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBTZCount(nCountLastUsed);
    else if (!walletdb.ReadZBTZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzcarbonWallet::Lock()
{
    seedMaster = 0;
}

void CzcarbonWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzcarbonWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBTZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzcarbonWallet::LoadMintPoolFromDB()
{
...
void CzcarbonWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzcarbonWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzcarbonWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcarbonTracker->HasPubcoinHash(pMint.first)) {
...
bool CzcarbonWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBTZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcarbonTracker which also adds to database
    pwalletMain->zcarbonTracker->Add(dMint, true);
...
void CzcarbonWallet::SeedToZBTZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzcarbonWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzcarbonWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZBTZCount(nCountLastUsed);
...
void CzcarbonWallet::GenerateDeterministicZBTZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzcarbonWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzcarbonWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
