...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zcarbonTracker = unique_ptr<CzcarbonTracker>(new CzcarbonTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZBTZ_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZBTZ_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
