...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BTZ at the same time
...
enum ZerocoinSpendStatus {
    ZBTZ_SPEND_OKAY = 0,                            // No error
    ZBTZ_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBTZ_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBTZ_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBTZ_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBTZ_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBTZ_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBTZ_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBTZ_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBTZ_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBTZ_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBTZ_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBTZ_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBTZ_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBTZ_SPENT_USED_ZBTZ = 14,                      // Coin has already been spend
    ZBTZ_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBTZ_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZBTZOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzcarbonAuto) const;
    ...
    */
    ...
    CzcarbonWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzcarbonTracker> zcarbonTracker;
    ...
    void setZWallet(CzcarbonWallet* zwallet)
    {
        zwalletMain = zwallet;
        zcarbonTracker = std::unique_ptr<CzcarbonTracker>(new CzcarbonTracker(strWalletFile));
    }

    CzcarbonWallet* getZWallet() { return zwalletMain; }
    ...
    /** zcarbon reset */
    boost::signals2::signal<void()> NotifyzcarbonReset;
