...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BTZ)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sBTZPercentage, QString& szcarbonPercentage)
{
    int nPrecision = 2;
    ...
    sBTZPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelBTZPercent->setText(sPercentage);
    ...
    // BTZ Available
    bool showBTZAvailable = settingShowAllBalances || pivAvailableBalance != nTotalBalance;
    bool showWatchOnlyBTZAvailable = showBTZAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showBTZAvailable || showWatchOnlyBTZAvailable);
    ui->labelBalance->setVisible(showBTZAvailable || showWatchOnlyBTZAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlyBTZAvailable && showWatchOnly);

    // BTZ Pending
    bool showBTZPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyBTZPending = showBTZPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showBTZPending || showWatchOnlyBTZPending);
    ui->labelUnconfirmed->setVisible(showBTZPending || showWatchOnlyBTZPending);
    ui->labelWatchPending->setVisible(showWatchOnlyBTZPending && showWatchOnly);

    // BTZ Immature
    bool showBTZImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showBTZImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showBTZImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showBTZImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // BTZ Locked
    bool showBTZLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyBTZLocked = showBTZLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showBTZLocked || showWatchOnlyBTZLocked);
    ui->labelLockedBalance->setVisible(showBTZLocked || showWatchOnlyBTZLocked);
    ui->labelWatchLocked->setVisible(showWatchOnlyBTZLocked && showWatchOnly);
    ...
    ui->labelBTZPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BTZ")
    updateDisplayUnit();
}
