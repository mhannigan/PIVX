...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BTZ);
    ...
    if (!settings.contains("nAnonymizeCarbonZeroAmount"))
        settings.setValue("nAnonymizeCarbonZeroAmount", 1000);

    nAnonymizeCarbonZeroAmount = settings.value("nAnonymizeCarbonZeroAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCarbonZeroAmount"))
        SoftSetArg("-anonymizecarbonzeroamount", settings.value("nAnonymizeCarbonZeroAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in carbonzero.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCarbonZeroAmount:
            return QVariant(nAnonymizeCarbonZeroAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCarbonZeroAmount:
            nAnonymizeCarbonZeroAmount = value.toInt();
            settings.setValue("nAnonymizeCarbonZeroAmount", nAnonymizeCarbonZeroAmount);
            emit anonymizeCarbonZeroAmountChanged(nAnonymizeCarbonZeroAmount);
            break;
        case CoinControlFeatures:
