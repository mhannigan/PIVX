...
// The zcarbon block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zcarbon is a hash of the serial
...
{
    //Create an output returning the zcarbon that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBTZOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zcarbon output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zcarbon", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBTZOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zcarbon output", __func__);
...
{
    CzcarbonTracker* zcarbonTracker = pwallet->zcarbonTracker.get();
    CMintMeta meta;
    if (!zcarbonTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcarbonTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BTZ stake is the outpoint
