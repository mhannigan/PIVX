...
#ifndef CarbonZero_ZBTZWALLET_H
#define CarbonZero_ZBTZWALLET_H
...
public:
    CzcarbonWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBTZ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBTZ(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //CarbonZero_ZBTZWALLET_H
